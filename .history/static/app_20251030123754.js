...
function createCigarCard(cigar) {
    ...
    // SVG cigar icon for placeholder
    const cigarIconSvg = `
        <svg viewBox="0 0 24 24" style="width: 80px; height: 80px; opacity: 0.4;">
            <path fill="currentColor" d="M20,13C20,14.4 19.8,15.7 19.4,17L15.6,14.2C15.9,13.5 16,12.8 16,12C16,11.2 15.9,10.5 15.6,9.8L19.4,7C19.8,8.3 20,9.6 20,11M4,11C4,9.6 4.2,8.3 4.6,7L8.4,9.8C8.1,10.5 8,11.2 8,12C8,12.8 8.1,13.5 8.4,14.2L4.6,17C4.2,15.7 4,14.4 4,13M12,4C13.4,4 14.7,4.2 16,4.6L13.2,8.4C12.5,8.1 11.8,8 11,8C10.2,8 9.5,8.1 8.8,8.4L6,4.6C7.3,4.2 8.6,4 10,4M12,20C10.6,20 9.3,19.8 8,19.4L10.8,15.6C11.5,15.9 12.2,16 13,16C13.8,16 14.5,15.9 15.2,15.6L18,19.4C16.7,19.8 15.4,20 14,20M12,12A2,2 0 0,1 14,10A2,2 0 0,1 16,12A2,2 0 0,1 14,14A2,2 0 0,1 12,12Z" />
        </svg>
    `;
    
    // Determine image source or use placeholder
    const imageHtml = cigar.image_url 
        ? `<img src="${cigar.image_url}" alt="${cigar.name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
           <div class="cigar-card-image-placeholder" style="display: none;">${cigarIconSvg}</div>`
        : `<div class="cigar-card-image-placeholder">${cigarIconSvg}</div>`;
