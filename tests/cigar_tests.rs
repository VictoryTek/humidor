...
async fn test_list_all_cigars() {
    ...
    create_test_cigar(&ctx.pool, "Cigar 1", 5, None).await.unwrap();
    create_test_cigar(&ctx.pool, "Cigar 2", 3, None).await.unwrap();
    create_test_cigar(&ctx.pool, "Cigar 3", 7, None).await.unwrap();
...
async fn test_filter_cigars_by_humidor() {
    ...
    create_test_cigar(&ctx.pool, "Cigar A", 5, Some(humidor1_id)).await.unwrap();
    create_test_cigar(&ctx.pool, "Cigar B", 3, Some(humidor1_id)).await.unwrap();
    create_test_cigar(&ctx.pool, "Cigar C", 7, Some(humidor2_id)).await.unwrap();
