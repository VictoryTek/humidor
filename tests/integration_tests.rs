...
async fn test_humidor_user_isolation() {
    ...
    // Create two users
    let (user1_id, _) = create_test_user(&ctx.pool, "user1", "password", false)
        .await
        .unwrap();
    let (user2_id, _) = create_test_user(&ctx.pool, "user2", "password", false)
        .await
        .unwrap();

    // Each creates a humidor
    let humidor1_id = create_test_humidor(&ctx.pool, user1_id, "User 1 Humidor")
        .await
        .unwrap();
    let humidor2_id = create_test_humidor(&ctx.pool, user2_id, "User 2 Humidor")
        .await
        .unwrap();
    ...
    let row1 = client
        .query_one(
            "SELECT user_id FROM humidors WHERE id = $1",
            &[&humidor1_id],
        )
        .await
        .unwrap();
...
async fn test_cigar_quantity_tracking() {
    ...
    client
        .execute(
            "UPDATE cigars SET quantity = $1 WHERE id = $2",
            &[&7, &cigar_id],
        )
        .await
        .unwrap();
...
async fn test_cigar_humidor_relationship() {
    ...
    let (user_id, _) = create_test_user(&ctx.pool, "cigar_user", "password", false)
        .await
        .unwrap();
    let humidor_id = create_test_humidor(&ctx.pool, user_id, "Cigar Humidor")
        .await
        .unwrap();
...
async fn test_favorite_persistence() {
    ...
    let (user_id, _) = create_test_user(&ctx.pool, "fav_user", "password", false)
        .await
        .unwrap();
    let cigar_id = create_test_cigar(&ctx.pool, "Favorite Cigar", 3, None)
        .await
        .unwrap();
...
async fn test_wish_list_with_notes() {
    ...
    let (user_id, _) = create_test_user(&ctx.pool, "wish_user", "password", false)
        .await
        .unwrap();
    let cigar_id = create_test_cigar(&ctx.pool, "Wish Cigar", 0, None)
        .await
        .unwrap();
...
async fn test_favorite_unique_constraint() {
    ...
    let (user_id, _) = create_test_user(&ctx.pool, "unique_user", "password", false)
        .await
        .unwrap();
    let cigar_id = create_test_cigar(&ctx.pool, "Unique Cigar", 5, None)
        .await
        .unwrap();
...
async fn test_cascade_delete_user_humidors() {
    ...
    let (user_id, _) = create_test_user(&ctx.pool, "cascade_user", "password", false)
        .await
        .unwrap();
    let _humidor_id = create_test_humidor(&ctx.pool, user_id, "Will Be Deleted")
        .await
        .unwrap();
...
async fn test_user_admin_flag() {
    ...
    let (admin_id, _) = create_test_user(&ctx.pool, "admin", "password", true)
        .await
        .unwrap();
    let (user_id, _) = create_test_user(&ctx.pool, "regular", "password", false)
        .await
        .unwrap();
